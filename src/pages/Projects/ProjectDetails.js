...
const ProjectDetails = () => {
    ...
    return (
        <section id='project-details' sx={{ minWidth: '80vh', display: 'flex'}}>
            <Box sx={{ minWidth: '80vh', textAlign: 'center', py: 4, backgroundColor: '#f0f0f0', marginTop: '10px', borderRadius: '15px', boxShadow: '0px 4px 10px rgba(0, 0, 0, 0.1)', mb: 6}}>
            ...
            <Box sx={{textAlign: 'center', backgroundColor: '#f0f0f0', borderRadius: '15px', boxShadow: '0px 4px 10px rgba(0, 0, 0, 0.1)', mb: 6 }}>
                ...
                <div style={{display: 'flex', justifyContent: 'center', alignItems: 'center'}}>
                    ...
                    {project.images.map((image, index) => (
                        <Box style={{overflow: 'hidden', backgroundSize: 'contain', borderRadius: '0.5rem', height: '60%', boxShadow: '0px 0px 7px #666', maxWidth: '60%', maxHeight: '400px'}}>
                            {image.media === 'image' ? ( 
                                <img className={currentImageIndex === index ? "slide" : "slide-hidden"} src={image.link} alt={image.name} key={index} style={{backgroundSize: 'contain', borderRadius: '0.5rem', height: '60%', maxWidth: '100%', maxHeight:'80%'}}></img>) :
                                <video className={currentImageIndex === index ? "slide" : "slide-hidden"} src={image.link} alt={image.name} controls key={index} style={{backgroundSize: 'contain', borderRadius: '0.5rem', height: '60%', maxWidth: '100%', maxHeight:'80%'}}></video>
                            }
                        </Box>
                    ))}
